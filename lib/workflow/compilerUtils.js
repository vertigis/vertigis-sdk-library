...
/**
 * @param {Node} node
 * @returns {node is import("ts-morph").ClassDeclaration}
 */
...
/**
 * @param {Node} node
 * @returns {node is import("ts-morph").VariableDeclaration}
 */
...
/**
 * @param {import("ts-morph").Expression} expression
 * @returns {expression is import("ts-morph").ObjectLiteralExpression}
 */
...
function getTagsFromJsDocs(jsDocs) {
    ...
    const tags = jsDocs
        .reduce(
            (/** @type {import("ts-morph").JSDocTag[]} */ acc, jsDoc) =>
                acc.concat(jsDoc.getTags()),
            []
        )
        .reduce(
            (acc, tag) => ({
...
/**
 * @param {Node} param
 * @param {import("ts-morph").Symbol[]} properties
...
function getParameterMetadata(param, properties, inputType) {
    ...
    for (const paramProp of properties) {
        ...
        const paramTags = paramProp.compilerSymbol.getJsDocTags().reduce(
            (acc, tag) => ({
                ...acc,
                [tag.name]: tag.text?.[0].text ?? tag.text,
            }),
            {}
        );
        ...
        const paramMeta = {
            ...
            name: paramName,
            noExpressions:
                Object.prototype.hasOwnProperty.call(paramTags, "noExpressions") || undefined,
...
/**
 * @param {import("ts-morph").ClassDeclaration} declaration
...
function getActivityMetadata(declaration, suiteUuid) {
    ...
    const inputs = inputType
        ? getParameterMetadata(executeMember, inputType.getProperties(), "activity input")
        : {};
...
/**
 * @param {import("ts-morph").VariableDeclaration} registrationDeclaration
 * @param {string} suiteUuid
 */
function getElementMetadata(registrationDeclaration, suiteUuid) {
    ...
    /**
     * @param {import("ts-morph").ObjectLiteralExpression} regObject
     */
    ...
    /**
     *
     * @param {import("ts-morph").ClassDeclaration | import("ts-morph").VariableDeclaration | import("ts-morph").FunctionDeclaration} propValue
     * @returns {import("ts-morph").ArrowFunction | import("ts-morph").FunctionDeclaration | import("ts-morph").ClassDeclaration}
     */
    ...
    /**
     * @param {import("ts-morph").ArrowFunction | import("ts-morph").FunctionDeclaration | import("ts-morph").ClassDeclaration | import("ts-morph").CallSignatureDeclaration} componentDeclaration
     * @returns {{}}
     */
    function getInputsMetadata(componentDeclaration) {
         ...
         * @param {import("ts-morph").Symbol[]} members
         */
 ...
 * project.
 * @param {import("ts-morph").SourceFile} projectExportsFile
 * @param {string} suiteUuid
 */
